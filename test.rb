...
class TestWaterslide < MiniTest::Test
  def test_piping_a_scalar_through_no_op
    assert_equal 1, (Filter[1] >> NoOp).first
  end

  def test_piping_a_scalar_through_multiple_no_ops
    assert_equal 1, (Filter[1] >> NoOp >> NoOp).first
  end

  def test_piping_a_scalar_through_add_one
    assert_equal 2, (Filter[1] >> AddOne).first
  end

  def test_piping_a_scalar_through_multiple_add_ones
    assert_equal 3, (Filter[1] >> AddOne >> AddOne).first
  end

  def test_piping_an_array_through_no_op
    assert_equal [1,2,3], (Filter[[1,2,3]] >> NoOp).all
  end

  def test_piping_an_array_through_add_one
    assert_equal [2,3,4], (Filter[[1,2,3]] >> AddOne).all
  end

  def test_piping_an_array_through_multiple_add_ones
    assert_equal [3,4,5], (Filter[[1,2,3]] >> AddOne >> AddOne).all
  end

  def test_piping_an_array_through_add
    assert_equal [4,5,6], (Filter[[1,2,3]] >> Add(3)).all
  end

  def test_that_pipes_are_enumerables
    assert (Filter[[1,2,3]] >> Add(3)).include? 4
    assert_equal 3, (Filter[[1,2,3]] >> Add(3)).count
  end
  ...
  def test_duplicate
    assert_equal [1,1,2,2,3,3], (Filter[[1,2,3]] >> Duplicate).all
  end
  ...
  def test_piping_an_array_through_a_filter
    assert_equal [2,4,6], (Filter[[1,2,3,4,5,6]] >> OnlyEvens).all
  end
  ...
  def test_sorting_with_overridden_incoming
    assert_equal [1,2,3,4,5], (Filter[[4,1,5,3,2]] >> Sort).all
  end
  ...
  def test_subclasses_of_pipes
    assert_equal [5,4,3,2,1], (Filter[[4,1,5,3,2]] >> Sort::Descending).all
  end
  ...
  def test_incoming_with_above_average
    assert_equal [4,5], (Filter[[1,2,3,4,5]] >> AboveAverage).all
  end
